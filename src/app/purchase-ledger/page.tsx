...
import { getPurchaseParties, createPurchaseParty, deletePurchaseParty, createPurchaseEntry, getLatestPurchaseEntries } from "@/lib/actions"
import { PaginationControls } from "@/components/common/PaginationControls"
import { numberToWordsIN } from "@/lib/utils"
import { Search, Plus, Building2, ArrowUpRight, ArrowDownRight, FileText, ChevronRight, X, IndianRupee, History, Trash2 } from "lucide-react"
...
export default function PurchaseLedgerPage() {
  ...
  const loadData = async () => {
    setLoading(true)
    const data = await getPurchaseParties()
    setParties(data || [])
  ...
  const handleDeleteParty = async (party: any) => {
    if (!confirm(`Are you sure you want to delete "${party.name}"? This will permanently remove the supplier and all associated ledger entries.`)) return
    try {
      const res = await deletePurchaseParty(party.id)
      if (res && !res.success) {
        alert(res.error)
      } else {
        await loadData()
  ...
  const handleOpenHistory = async () => {
    const data = await getLatestPurchaseEntries()
    ...
    <div className="w-full pb-20 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="typo-h1 flex items-center gap-2.5">
            <Building2 className="w-8 h-8 text-[#2FA084]" /> Purchase Ledger
          </h1>
          <p className="typo-body text-[#111111]/60 mt-1">Manage supplier accounts, track payables, and record purchases or advances.</p>
        </div>
                    ...
                    {isPartyDropdownOpen && (
                      <div className="absolute z-50 left-0 right-0 mt-1 max-h-48 overflow-y-auto rounded-xl bg-white border border-gray-200 shadow-xl divide-y divide-gray-100">
                        {parties.filter(p => p.name.toLowerCase().includes(partySearchText.toLowerCase())).length === 0 ? (
                          <div className="p-3 text-xs text-gray-400 text-center">No matching parties</div>
                        ) : (
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.82)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', border: '1px solid rgba(255,255,255,0.65)', boxShadow: '0 1px 0 rgba(255,255,255,0.90) inset, 0 -1px 0 rgba(0,0,0,0.05) inset, 0 6px 24px rgba(0,0,0,0.08), 0 2px 6px rgba(0,0,0,0.05)' }}>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                <tr>
                  <td colSpan={6} className="py-20 text-center text-[#111111]/40 font-medium">Loading purchase ledger...</td>
                </tr>
                ...
                <tr>
                  <td colSpan={6} className="py-20 text-center text-[#111111]/40">
                    ...
                    <p className="text-xs mt-1">Click &quot;+ Add a Party&quot; to start maintaining your supplier ledger.</p>
                  </td>
                </tr>
              ) : (
                (() => {
        ...
        <div className="md:hidden divide-y divide-gray-200 border-t border-gray-200">
          {loading ? (
            <div className="p-12 text-center text-[#111111]/40 font-medium">Loading purchase ledger...</div>
          ) : filteredParties.length === 0 ? (
