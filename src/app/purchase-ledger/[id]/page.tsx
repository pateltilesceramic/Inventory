...
import { getPurchasePartyById, createPurchaseEntry, deletePurchaseEntry, updatePurchaseParty, updatePurchaseEntry } from "@/lib/actions"
import { numberToWordsIN } from "@/lib/utils"
import { SecurityGate } from "@/components/SecurityGate"
import { PaginationControls } from "@/components/common/PaginationControls"
import { ArrowLeft, Plus, Trash2, Building2, Calendar, FileText, IndianRupee, X, Edit2, Printer } from "lucide-react"
import { motion, AnimatePresence } from "framer-motion"

export default function PurchasePartyDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const loadData = async () => {
    setLoading(true)
    const data = await getPurchasePartyById(id)
    setParty(data)
    setLoading(false)
  }
  ...
  const handleUpdateParty = async (e: React.FormEvent) => {
    ...
    triggerSecurity(async () => {
      setIsUpdating(true)
      try {
        await updatePurchaseParty(id, editPartyName)
  ...
  const handleUpdateEntry = async (e: React.FormEvent) => {
    ...
    triggerSecurity(async () => {
      setIsUpdating(true)
      try {
        await updatePurchaseEntry(editingEntry.id, id, {
  ...
  const handleDelete = (entryId: string) => {
    ...
    triggerSecurity(async () => {
      await deletePurchaseEntry(entryId, id)
      await loadData()
    })
  }
  ...
  if (!party) {
    return (
      <div className="w-full py-20 text-center max-w-6xl mx-auto space-y-4">
        <p className="font-bold text-lg text-gray-600">Party not found.</p>
        <Link href="/purchase-ledger" className="text-[#1F6F5F] font-bold hover:underline">
          &larr; Back to Purchase Ledger
        </Link>
      </div>
    )
  }
  ...
  return (
    <div className="w-full pb-20 max-w-6xl mx-auto space-y-8">
      {/* Top Nav & Header */}
      <div>
        <Link href="/purchase-ledger" className="inline-flex items-center gap-1.5 text-xs font-bold text-[#1F6F5F] hover:underline mb-4">
          <ArrowLeft className="w-4 h-4" /> Back to Purchase Ledger
        </Link>
          ...
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="typo-body text-gray-500 mt-0.5">Supplier / Purchase Account Statement</p>
              </div>
            </div>
          </div>
              ...
              {party.entries.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-16 text-center text-[#111111]/40">
                    ...
                    <p className="text-xs mt-1">Click &quot;Add Entry&quot; to log your first purchase or payment.</p>
                  </td>
                </tr>
              ) : (
                (() => {
        ...
        {isEditPartyOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(10,30,25,0.55)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }}>
            ...
            >
              <div className="flex justify-between items-center pb-4 mb-4 border-b border-gray-100">
                <h3 className="text-lg font-bold text-[#1F6F5F] flex items-center gap-2">
                  <Edit2 className="w-5 h-5 text-[#2FA084]" /> Edit Supplier Name
                </h3>
          ...
          <div className="border-b-2 border-black pb-3 mb-3">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-xs font-bold text-gray-600 mt-0.5">Supplier / Purchase Account Statement</p>
              </div>
