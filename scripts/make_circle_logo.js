...
fs.createReadStream('public/logo-emblem.png')
  ...
  .on('parsed', function() {
    ...
    const cx = 250, cy = 250;
    const circleRadius = 249; // Edge-to-edge solid white circle (498px diameter)
    const targetSize = 476;   // Scale logo up to 476px, keeping a crisp ~11px white separation ring!
    ...
    outPNG.pack().pipe(fs.createWriteStream('public/logo-qr-circle.png'))
      .on('finish', () => {
        console.log('Successfully generated 476px target size logo-qr-circle.png!');
      });
  });
